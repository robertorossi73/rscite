...
LUALIB_API int c_MsgBox(lua_State *L)
{
  ...
  if ((n > 0) && (n < 4) && (lua_type(L,1)==LUA_TSTRING)) {    
    ...
    showErrorMsg("Argomenti errati! - MsgBox(messaggio [,titolo [,flag])");
    lua_error(L); 
    lua_pushnil(L);
...
LUALIB_API int c_fileOperation(lua_State *L)
{
      ...
      (n == 4))
  {
    ...
    buffer = calloc(strlen(from)+2,sizeof(char));
    strcpy(buffer,from);
...
/*scrive nel file temporaneo indicato il contenuto di un valore stringa presente nel registro*/
LUALIB_API int c_RegGetString(lua_State *L)
{
  char szBuffer[1024];       // buffer
  const int n = lua_gettop(L);
  const char *key, *fileName, *mainKey;
  ...
  if ((lua_type(L,1)==LUA_TSTRING) && 
      ...
      (n > 2))
  {
    key = lua_tostring(L,1); //percorso valore
		mainKey = lua_tostring(L,2); //chiave principale
		fileName = lua_tostring(L,3); //nome file scambio temporaneo
    ...
    nError = RegQueryValueEx(hKey, key, 0, NULL, (LPBYTE)szBuffer, &dwBufferSize);
    if (ERROR_SUCCESS == nError)
    {
			writeStrinToTmp(szBuffer, fileName);
			lua_pushboolean(L, 1);
    } else {
      lua_pushnil(L);
    }
  } else {
		showErrorMsg("Missing Arguments! - RegGetString(ValueName, mainKey, TempFileName)");
    lua_error(L); 
    lua_pushnil(L);
  }
  return 1;
