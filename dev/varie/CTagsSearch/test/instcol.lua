...
do
  ...
  -- 99=[Testo da aggiungere]
  -- 100=\r\n 1 -> Inizio linea \r\n 0 -> Fine linea
  -- 101=Testo da Inserire...
  -- 102=Specifica il testo da inserire :
  -- 103=Inserimento Testo in Colonna...
  -- 104=Specifica la colonna nella quale inserire il testo :
  local testoAdd = _t(99)
  local msg = _t(100)

  testoAdd = rwfx_InputBox("", _t(101),_t(102),rfx_FN())
  if (testoAdd) then
    testoAdd = rfx_GF()
    lung = rwfx_InputBox("1", _t(103),_t(104)..msg,rfx_FN())
    if lung then
      lung = rfx_GF()
      lung = tonumber(lung)
      if (lung > -1) then
