...
LUALIB_API int c_RegGetString(lua_State *L)
{
      ...
      (n == 4))
  {
    ...
    valueName = lua_tostring(L,2);
    mainKey = lua_tostring(L,3); //chiave principale es: HKEY_CURRENT_USER
    fileName = lua_tostring(L,3); //output filename
...
LUALIB_API int c_Test(lua_State *L)
{
	...
	strcpy(datai, "Argument 1;Argument 2;Argument 3;Argument 4");
	start_ShowHTMLDialog(GetActiveWindow(),
						"d:\\sv\\Projects\\scite\\dev\\sources\\utilities\\rluawfx_v3\\Debug\\test.html",
