--[[Invia il testo selezionato a Mistral AI
Version : 1.0.0
Author  : Roberto Rossi
Web     : http://www.redchar.net

Invia una domanda(il testo selezionato) a Mistral
...
do
  ...
  local function save_expressions(values)
    local nomef = rfx_UserFolderRSciTE()..'/tmp/exeMistral.ini'
    local v
    local count = 1
    ...
    while (count < 26) do
      v = values[count]
      if (v) then
        rfx_setIniVal(nomef, "General", "mistral"..tostring(count) , v)
      end
      count = count + 1
    end
  end
  ...
  local function get_expressions()
    ...
    while(i < 26) do
      v = rfx_GetIniVal(nomef, "General", "mistral"..tostring(i))
      if (v ~= "") then
        result[i] = v
      else
        break
  ...
  function buttonOk_click(control, change)
    local values
    local text = ""
    local service = "https://chat.mistral.ai/chat?q="
    local command = ""
