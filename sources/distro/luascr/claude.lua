--[[Invia il testo selezionato a Claude AI
Version : 1.0.0
Author  : Roberto Rossi
Web     : http://www.redchar.net

Invia una domanda(il testo selezionato) a Claude
...
do
  ...
  local function save_expressions(values)
    local nomef = rfx_UserFolderRSciTE()..'/tmp/exeClaude.ini'
    local v
    local count = 1
    ...
    while (count < 26) do
      v = values[count]
      if (v) then
        rfx_setIniVal(nomef, "General", "claude"..tostring(count) , v)
      end
      count = count + 1
    end
  end
  ...
  local function get_expressions()
    ...
    while(i < 26) do
      v = rfx_GetIniVal(nomef, "General", "claude"..tostring(i))
      if (v ~= "") then
        result[i] = v
      else
        break
  ...
  function buttonOk_click(control, change)
    local values
    local text = ""
    local service = "https://claude.ai/new?q="
    local command = ""
